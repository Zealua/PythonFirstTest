...
//printf("Ignore other printouts\n");
  ...
  char *barcode_device;
  if (argc == 2) {
     barcode_device = argv[1];
     //barcode_device = "/dev/input/by-id/usb-ﾩSymbol_Technologies__Inc__2002_Symbol_Bar_Code_Scanner_S_N:2EE81115DFE76340AE4AFBF0C18B5FED_Rev:NBRLEAAEDM:06OCT093-event-kbd";
  } else {
     printf("Usage: %s <device file>\n", argv[0]);
     return 1;
  }

  if ((fd = open(barcode_device, O_RDONLY)) < 0) {
    printf("Error opening file descriptor %s\n", barcode_device);
    return 3;
  }
  //printf("Opening barcode reader device %s\n", barcode_device);
